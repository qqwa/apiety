...
impl ProcessMemory {
    ...
    pub fn search(&self, magic: &[u8], size: usize, min_size: u64) -> Vec<Vec<u8>> {
        use winapi::shared::minwindef::{DWORD, BOOL, LPCVOID, LPVOID};
        ...
        use winapi::um::winnt::{PROCESS_VM_READ, PROCESS_QUERY_INFORMATION, MEMORY_BASIC_INFORMATION, PVOID };
        use winapi::um::errhandlingapi::GetLastError;

        let handle = unsafe {
            OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, false as BOOL, self.pid as u32)
        };
        ...
        loop {
            let res = unsafe {
                VirtualQueryEx(handle, address as LPCVOID , &mut info, std::mem::size_of::<MEMORY_BASIC_INFORMATION>())
            };
            if res == 0 {
            ...
            let mut page_info: PSAPI_WORKING_SET_EX_INFORMATION = unsafe { std::mem::uninitialized() };
            page_info.VirtualAddress = info.BaseAddress;
            // returns 0 if it fails
            let _ = unsafe {
                QueryWorkingSetEx(handle, &mut page_info as *mut PSAPI_WORKING_SET_EX_INFORMATION as PVOID, std::mem::size_of::<PSAPI_WORKING_SET_EX_INFORMATION>() as u32)
            };
            ...
            if page_info.VirtualAttributes.Valid() != 0 && min_size as usize <= info.RegionSize {
                ...
                let mut bytes = 0;
                unsafe {
                    ReadProcessMemory(handle, address as LPCVOID, buffer.as_mut_ptr() as LPVOID, info.RegionSize as usize, &mut bytes);
                }
